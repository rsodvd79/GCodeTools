...
Public Class frmMain
    ...
    Private Sub Modo4()
        ...
        For LI As Integer = 0 To Segmenti.Count - 1
            ...
            stbR.AppendLine($"   translate([0, 0-(Ugello/2), 0]) cube([{sg.Distanza.ToString(CultureInfo.InvariantCulture)}, Ugello, Layer]);")

            If sg.Primo Then
                stbR.AppendLine($"   translate([0,0,0]) cylinder(d=Ugello, h=Layer);")

            End If

            stbR.AppendLine($"   translate([{sg.Distanza.ToString(CultureInfo.InvariantCulture)},0,0]) cylinder(d=Ugello, h=Layer);")
            stbR.AppendLine("};")

            If sgn IsNot Nothing AndAlso Not sgn.Primo Then
                stbR.AppendLine($"translate({sgn.Origine.aStringaComplessa()}) rotate([0, 0, {sgn.Angolo.ToString(CultureInfo.InvariantCulture)}]) translate([0, 0-(Ugello/2), 0]) cube([{sg.Distanza.ToString(CultureInfo.InvariantCulture)}, Ugello, Layer]);")
                stbR.AppendLine("};")
