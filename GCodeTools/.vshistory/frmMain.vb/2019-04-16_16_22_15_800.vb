...
Public Class frmMain
    ...
    Public Sub ControllaProcessi()
        ...
        If intCntProcessiAttivi = 0 AndAlso Processi.Count > 0 Then
            ...
            Testo($"{vbCrLf}STL FINITI ...  INCORPORO ...")
            ...
            For Each p As clsProcesso In Processi
                If IO.File.Exists(p.FileStl) Then
                    ...
                    For Each strZ As String In IO.File.ReadAllLines(p.FileStl)
                        If Not strZ.Trim.ToUpper.StartsWith("solid") AndAlso Not strZ.Trim.ToUpper.StartsWith("ENDSOLID") AndAlso Not String.IsNullOrEmpty(strZ) Then
                            stbX.Append(strZ)
    ...
    Private Sub Modo4()
        ...
        For LI As Integer = 0 To Segmenti.Count - 1
            ...
            AddHandler Processi.Last.Finito, Sub(ps, pe)
                                                 ...
                                                 '    Dim stbX As New StringBuilder()
                                                 '    stbX.AppendLine("solid XXX")

                                                 '    For Each p As clsProcesso In Processi
                                                 '        If IO.File.Exists(p.FileStl) Then
                                                 '            Dim strS As String = IO.File.ReadAllText(p.FileStl)

                                                 '            strS = strS.Replace("solid OpenSCAD_Model", String.Empty)
                                                 '            strS = strS.Replace("endsolid OpenSCAD_Model", String.Empty)
                                                 ...
                                                 '    stbX.AppendLine("endsolid XXX")

                                                 '    IO.File.WriteAllText(Processi.Last.FileStl, stbX.ToString)
