...
Public Class frmMain
    ...
    Private Sub Modo4()
        ...
        For LI As Integer = 0 To Segmenti.Count - 1
            ...
            stbR.AppendLine("translate(Origine) rotate([0, 0, Angolo]) union () {")
            stbR.AppendLine($"   translate([0, 0-(Ugello/2), 0]) cube([Distanza, Ugello, Layer]);")
            ...
            stbR.AppendLine($"   translate([Distanza,0,0]) cylinder(d=Ugello, h=Layer);")
            stbR.AppendLine("};")

            If sgn IsNot Nothing AndAlso Not sgn.Primo Then
                stbR.AppendLine($"translate({sgn.Origine.aStringaComplessa()}) rotate([0, 0, {sg.Angolo.ToString(CultureInfo.InvariantCulture)}]) translate([0, 0-(Ugello/2), 0]) cube([{sg.Distanza.ToString(CultureInfo.InvariantCulture)}, Ugello, Layer]);")
                stbR.AppendLine("};")
