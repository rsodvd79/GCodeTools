...
Public Class frmMain
    ...
    Private Sub BackgroundWorker_Crea_Segmenti_RunWorkerCompleted(sender As Object, e As RunWorkerCompletedEventArgs) Handles BackgroundWorker_Crea_Segmenti.RunWorkerCompleted
        ...
        ElseIf arg.Tipo = BackgroundWorker_Crea_STL_Argument.TipoCreaSegmenti.Per_SCAD Then
            ...
            Dim Segmenti_Count As Integer = Segmenti.Count - 1

            'For Each sg As clsSegmento In Segmenti
            For intF As Integer = Math.Min(Ignora_Segmenti, Segmenti_Count) To Segmenti_Count
                If blnPrimo Then
                    blnPrimo = False
                ...
                Dim sg As clsSegmento = Segmenti(intF)
            ...
            For intF As Integer = Math.Min(Ignora_Segmenti, Segmenti_Count) To Segmenti_Count Step intS
                stbR.AppendLine($"for(i = [{intF.ToString()} : {Math.Min(Segmenti_Count, intF + intS - 1).ToString()}]) drw(gcd[i][0],gcd[i][1]);")
